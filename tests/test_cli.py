...
class TestCmdHelp:
    ...
    def test_help_returns_zero(self):
        """Verify help command returns success."""
        result = cli.cmd_help([])
        assert result == 0

    def test_help_output_contains_commands(self, capsys: pytest.CaptureFixture[str]):
        """Verify help output lists available commands."""
        cli.cmd_help([])
        captured = capsys.readouterr()
...
class TestCmdServer:
    ...
    def test_server_fails_without_registry(self, tmp_cwd: Path, monkeypatch: pytest.MonkeyPatch):
        """Verify server fails when no registry found and no args."""
        monkeypatch.setattr(cli, "find_registry", lambda: None)
...
class TestMain:
    ...
    def test_main_dispatches_to_help(self, monkeypatch: pytest.MonkeyPatch):
        """Verify main dispatches help command."""
        monkeypatch.setattr(sys, "argv", ["agent-tools", "help"])
    ...
    def test_main_dispatches_help_flag(self, monkeypatch: pytest.MonkeyPatch):
    ...
    def test_main_dispatches_h_flag(self, monkeypatch: pytest.MonkeyPatch):
        """Verify main handles -h flag."""
        monkeypatch.setattr(sys, "argv", ["agent-tools", "-h"])
